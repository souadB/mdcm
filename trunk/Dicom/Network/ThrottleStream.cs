...
namespace Dicom.Network {
	...
	public class ThrottleStream : Stream {
		...
		public long MaximumBytesPerSecond {
			...
			set {
				if (MaximumBytesPerSecond != value) {
					_maximumBytesPerSecond = Math.Max(4096, value);
					if (_maximumBytesPerSecond == 0)
						_bufferSize = int.MaxValue;
					else
						_bufferSize = (int)(_maximumBytesPerSecond / 4);
					Reset();
				}
			}
		}
		...
		public override int Read(byte[] buffer, int offset, int count) {
			...
			int read = 0;
			while (count > 0) {
				int size = Math.Min(count, BufferSize);
				count -= size;
				Throttle(size);
		...
		public override void Write(byte[] buffer, int offset, int count) {
			...
			while (count > 0) {
				int write = Math.Min(count, BufferSize);
				Throttle(write);
		...
		protected void Throttle(int bufferSizeInBytes) {
			...
			if (elapsedMilliseconds > 0) {
				...
				if (bps > _maximumBytesPerSecond) {
					...
					int toSleep = Math.Min((int)(wakeElapsed - elapsedMilliseconds), 1000);
			...
			} else if (_byteCount >= _maximumBytesPerSecond) {
				Thread.Sleep(1);
			}
		}
